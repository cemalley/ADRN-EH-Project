...
out <- SKAT.SSD.All(SSD.INFO, obj, method="SKAT")
res <- rbind(res, out$results)
Close_SSD()

res_file1 <- paste(a,"chr",chrom,"_SKAT_ADNA_731samples_with_covar_missense_all_genes_clean_new_qc.txt",sep="")
...
out2 <- SKAT.SSD.All(SSD.INFO, obj2, method="SKAT")
res2 <- rbind(res2, out2$results)
Close_SSD()

res_file2 <- paste(a,"chr",chrom,"_SKAT_ADNA_731samples_with_covar_missense_TGP_rare_novel_genes_clean_new_qc.txt",sep="")
...
out3 <- SKAT.SSD.All(SSD.INFO, obj3, method="SKAT")
res3 <- rbind(res3, out3$results)
Close_SSD()

res_file3 <- paste(a,"chr",chrom,"_SKAT_ADNA_731samples_with_covar_missense_damaging_genes_clean_new_qc.txt",sep="")
...
out4 <- SKAT.SSD.All(SSD.INFO, obj4, method="SKAT")
res4 <- rbind(res4, out4$results)
Close_SSD()

res_file4 <- paste(a,"chr",chrom,"_SKAT_ADNA_731samples_with_covar_missense_damaging_TGP_rare_novel_genes_clean_new_qc.txt",sep="")
